...
import com.test.doa.ComposeDao;

/**
 * Servlet implementation class ComposeAction
 */
@WebServlet("/ComposeAction")
public class ComposeAction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      List<?> fileItems = upload.parseRequest(request);  //formfied-3 (to,sub,msg) Non FormField-1
          ...
          {    
            ...
            // String contentType = fi.getContentType();
             filename=fi.getName();
              ...
              //create folder
              File f = new File(sc.getServletContext().getRealPath("/")+"upload/") ;
                 if(!f.exists())
                	 f.mkdir();
              // Write the file
              ...
              file = new File(sc.getServletContext().getRealPath("/")+"upload/"+sub+filename) ;
               fi.write( file ) ;
    ...
    int x= new ComposeDao().compose(to, user, subject, msg);
  if(x!=0)
  {
